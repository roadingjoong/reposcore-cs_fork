...
```bash
dotnet add Test/Test.csproj reference reposcore-cs.csproj
```
...
`Test.csproj`에 xUnit 패키지가 있고 테스트 파일 위치도 맞는데 오류가 난다면,  
...
현재 프로젝트는 `Test` 테스트 프로젝트를 통해 주요 로직을 검증합니다.
...
`dotnet test Test/Test.csproj`명령어를 실행한 테스트 결과는 다음과 같이 나올 수 있습니다.
```bash
